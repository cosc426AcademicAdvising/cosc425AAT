...
########Could I make this one label?##############

#Making label to hold student information
student_info_label = Label(frame, text="Student Information")
student_info_label.place(width=700, height=80)

#Making label to hold information then placing in label
var = StringVar()
label = Label(student_info_label.place(relx=0.3, rely=0.025))
# concept output
var.set(
        " "*8 + "Name : " + std_name + '\n' +
...
recent_label = tk.Label(frame, text="Selected Courses", bg='white', fg='black')
recent_label.place(relx=0.015, rely=0.025, relwidth=0.14, relheight=0.4)
selecte_label = tk.Label(frame, text="Recent Schedule", bg='white', fg='black')
